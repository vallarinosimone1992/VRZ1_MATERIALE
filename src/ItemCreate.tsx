...
export function ItemCreate({ profile, onDone }: { profile: Profile; onDone: () => void }) {
  ...
  const [branchId, setBranchId] = useState('comune')
  const [unitId, setUnitId] = useState('comune')
  ...
  const allowedBranches = profile.role === 'rs'
    ...
    : dataRefs.branches
  const units = dataRefs.units.filter((unit) => unit.branch_id === branchId)
  const squads = dataRefs.squads.filter((squad) => squad.unit_id === unitId)
    ...
    <main className="stack">
      <form className="panel admin-form" onSubmit={save}>
        ...
        <div className="form-grid two-columns">
          ...
          <label>Branca *<select value={branchId} onChange={(e) => changeBranch(e.target.value)}>{allowedBranches.map((branch) => <option key={branch.id} value={branch.id}>{branch.label}</option>)}</select></label>
